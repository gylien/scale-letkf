...
character*30::cfile='./data_inv/d1-4.txt'

character*30::ctitle1='Domain 1-4'
character*30::ctitle2=''

character*30::psfile='./figs/d1-4'
...
character*14:: ctrecread, ctobsread, ctgfsread, ctd1read, ctd2read, ctd3read, ctd4read
...
open(11,file=trim(cfile),form='formatted')
 do i=1,nrecmax
  read(11,*,iostat=ios) ctrecread, ctobsread, ctgfsread, ctd1read,ctd2read, ctd3read, ctd4read
 if (ios.ne.0)then
  exit
 else
  iloc=0
 do iplot=1,nplot
  if (ctrecread.eq.ctimes(iplot))then
   ...
   call dtime_hour (ctd4read,  ctrecread, dt_d4(iplot) ) 
  exit
  end if
 end do
! if (iloc.eq.0)then
...
!  write(*,*) 'check :: ctrecread',ctrecread
!  do iplot=1,nplot
!   write(*,*) ctimes(iplot)
!  end do
 exit
...
! **** Lines & markers ****
      ...
      do iplot=1,nplot-1
       if (irecs(iplot).eq.1.and.irecs(iplot+1).eq.1)then
          ...
          call uulinz (2,xxs(iplot:iplot+1),dt_d4(iplot:iplot+1)-0.05,1,16)
       end if
      end do
...
!      call uxsttl ('B','JST',0.0)
      ...
      call sgtxzv (vpr-0.01,vpt-0.09,'D4 init/bdy',0.014,0,1,14) !       
