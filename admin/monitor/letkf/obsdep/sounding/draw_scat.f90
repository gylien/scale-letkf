program draw_obs_dep
  ...
  integer,parameter::nsmpmax=10000
...
!         n,ielm,vlon,vlat,vlev,vdat,verr,ityp,vdif,iqc,vomb,voma
  ...
  ismp=0
  do iprof=1,nprof
   if(vlons(iprof).ge.120.0 .and. vlats(iprof).ge.20.0)then
   do ilev=1,iprof_nlev(iprof)
     if (vprof_lev(ilev,iprof).ge. 500.0 .and.vprof_lev(ilev,iprof).le. 800.0) then
       ismp=ismp+1
...
!     end do
      call uumrkz(nsmp,vsmp_obs(1:nsmp),vsmp_obs(1:nsmp)-vsmp_fm(1:nsmp),10,41,0.004)
      call uumrkz(nsmp,vsmp_obs(1:nsmp),vsmp_obs(1:nsmp)-vsmp_am(1:nsmp),10,21,0.004)
