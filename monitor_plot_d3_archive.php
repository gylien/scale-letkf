...
date_default_timezone_set('UTC');
  ...
  $Xright= $Xleft + $Xsize * ( $NcycleFuture - (1 - $NcyclePast) ) ;
...
/* Forecast D2 */	
 ...
 $stimes='' ;
 exec("cd data/d2 ;ls -l1d $nowY* | awk '{print $9 \" \" $8}' ",$stimes  ,$ret);
 ...
 foreach ($stimes as $line) {
  ...
  for  ( $i = (1 - $FcstPastLimit) ; $i <= 0 ; $i++ ) {
   ...
   if ( $MMDDHHD2 == $CycleMDH[$i] ) {
    ...
    ImageFilledRectangle($image,$Xloc,$Yrect,min($Xright,$Xloc+($HfcstD2 * 60 / $CycleMin * $Xsize)),$Yrecb,$green);
    ImageRectangle($image,$Xloc,$Yrect,min($Xright,$Xloc+($HfcstD2 * 60 / $CycleMin * $Xsize)),$Yrecb,$gray);
...
/* Forecast D3 */	
  ...
  $stimesD3='' ;
 exec("ls -l1 data/d3/ref_$dirname/ | grep -v total | awk '{print $9 \" \" $8}' ",$stimesD3  ,$ret);
 ...
 foreach ($stimesD3 as $line) {
  ...
  for  ( $i = (1 - $FcstPastLimit) ; $i <= $NcycleFuture ; $i++ ) {
   ...
   if ( $MMDDHH == $CycleMDH[$i] ) {
    ...
    ImageFilledRectangle($image,$Xloc,$Yrect,min($Xright,$Xloc+($HfcstD3 * 60 / $CycleMin * $Xsize)),$YrecbD3,$blue);
    ImageRectangle($image,$Xloc,$Yrect,min($Xright,$Xloc+($HfcstD3 * 60 / $CycleMin * $Xsize)),$YrecbD3,$gray);
...
$fh = fopen("/home/amemiya/public_html/scale/monitor/monitor_fcst_d3.txt", 'rb');
 while ( $line = fgets($fh)) {
...
/* MSM forecast */				       
 ...
 $stimes='';
 exec("ls -l1 data/msm_d3/ | awk '{print $9 \" \" $8}'",$stimes ,$ret);
...
for  ( $i = (1 - $FcstPastLimit) ; $i <= 0 ; $i++ ) {
   ...
   if ( $MMDDHH == $CycleMDH[$i] ) {
    ...
    ImageFilledRectangle($image,$Xloc,$Yrect,min($Xright,$Xloc+($Hfcst / $CycleH * $Xsize)),$Yrecb,$green);
    ImageRectangle($image,$Xloc,$Yrect,min($Xright,$Xloc+($Hfcst / $CycleH * $Xsize)),$Yrecb,$gray);
...
/* 5min refresh  */
  ...
  $Yrecb = 550 - $YsizeIntvFcst ;
/*
$sout='';
 exec("ls -l1 data/JMA_precip/nowcast_d3/archive/radar_* | head -n 1 | awk '{print $9}' |   rev | cut -d '/' -f 1 | rev | sed -e 's/[^0-9]//g' ",$sout  ,$ret);
$times_anal_oldest=$sout[0];

$sout='';
 exec("ls -l1 data/JMA_precip/nowcast_d3/archive/radar_* | tail -n 1 | awk '{print $9}' |   rev | cut -d '/' -f 1 | rev | sed -e 's/[^0-9]//g' ",$sout  ,$ret);
$times_anal_latest=$sout[0];
...
#$TimeUAnalOldest=max($TimeUAnalOldest,$TimeUXleft);
#$TimeUAnalLatest=max($TimeUAnalLatest,$TimeUXright);
$TimeUAnalOldest=$TimeUXleft;
$TimeUAnalLatest=$TimeUXright;
$TimeUintv=$TimeUAnalOldest-$TimeUXleft;
$TimeUlength=$TimeUAnalLatest-$TimeUAnalOldest;

$Xloc=$Xleft+($TimeUintv *$Xsize / $CycleSecond);
$Yrect = $Yrecb + $YsizeIntvFcst ;
$Yrecb = $Yrect + $YsizeBarJMA ;
 ImageFilledRectangle($image,$Xloc,$Yrect,$Xloc+($TimeUlength / $CycleSecond * $Xsize),$Yrecb,$green);
 ImageRectangle($image,$Xloc,$Yrect,$Xloc+($TimeUlength / $CycleSecond * $Xsize),$Yrecb,$gray);
...
/* Legends */
  ...
  ImageString($image, 5, 10, 350, 'MSM' , $black);
  ImageString($image, 5, 10, 555, 'JMA_precip' , $black);
...
#  ImageString($image, 5, $Xloc, $Ynow, 'now'.' '.$nowH , $red);
  ...
  ImagePNG($image,'./monitor/archive/monitor_plot_d3_'.$basetime.'.png');
  ImageDestroy($image);


  $success = file_put_contents('./json/archive/xlocs_d3_'.$basetime.'.json', json_encode($XlocMarker)); 
  $success = file_put_contents('./json/archive/ylocs_d3_'.$basetime.'.json', json_encode($YlocMarker)); 
  $success = file_put_contents('./json/archive/itimes_d3_'.$basetime.'.json',  json_encode($InitTimes)); 
  $success = file_put_contents('./json/archive/ptimes_d3_'.$basetime.'.json', json_encode($ParentTimes)); 
