...
for  ( $i = (1 - $FcstPastLimit) ; $i <= 0 ; $i++ ) {
    ...
    $MMDDHH = $CycleMDH[$i] ;
###
   if ( file_exists( 'data/gfs/'.substr($timeinit,0,4).$MMDDHH.'0000' ) ) {
###
    $Yrect = $Yrecb + $YsizeIntvFcst ;
    $Yrecb = $Yrect + $YsizeBarFcst ;
...
#  ImageString($image, 5, 10, 143, 'GFS_analysis', $black);
  ImageString($image, 5, 10, 103, 'GFS_forecast', $black);
#  ImageString($image, 5, 10, 313, 'Analysis' , $black);
  ImageString($image, 5, 10, 253, 'Forecast' , $black);
...
#  ImageString($image, 5, $Xloc, $Ynow, 'now'.' '.$nowH , $red);
  ...
  ImagePNG($image,'./monitor/archive/monitor_plot_d1_'.$timeinit.'.png');
  ImageDestroy($image);

  $success = file_put_contents('json/archive/xlocs_'.$timeinit.'.json', json_encode($XlocMarker)); 
  $success = file_put_contents('json/archive/ylocs_'.$timeinit.'.json', json_encode($YlocMarker)); 
  $success = file_put_contents('json/archive/itimes_'.$timeinit.'.json', json_encode($InitTimes)); 
