...
$stimes='';
 exec("ls -l1 data/d2/ | tail -n 8 | awk '{print $9 \" \" $8}'",$stimes  ,$ret);
...
/* MSM forecast */				       
 ...
 $stimes='';
 exec("ls -l1 data/msm/ | tail -n 20 | awk '{print $9 \" \" $8}'",$stimes ,$ret);
...
/*  ImageFilledRectangle($image,5,1,995,55,$white); */
  ...
  ImagePNG($image,'./monitor/monitor_plot_d2.png');
  ImageDestroy($image);

  $success = file_put_contents('./json/xlocs_d2.json', json_encode($XlocMarker)); 
  $success = file_put_contents('./json/ylocs_d2.json', json_encode($YlocMarker)); 
  $success = file_put_contents('./json/itimes_d2.json', json_encode($InitTimes)); 
