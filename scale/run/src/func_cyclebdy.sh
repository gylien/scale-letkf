...
Usage: $myname [STIME ETIME MEMBERS CYCLE CYCLE_SKIP ISTEP FSTEP TIME_LIMIT]
...
echo "               $bdy_startframe"
          ...
          bash $SCRP_DIR/src/pre_scale_init.sh $MYRANK $mem_np \
               ...
               ${stimes[$c]} $CYCLEFLEN $mkinit ${name_m[$m]} \
          ...
          bash $SCRP_DIR/src/pre_scale_init.sh $MYRANK $mem_np \
               ...
               ${stimes[$c]} $CYCLEFLEN $mkinit mean \
               $TMPRUN/scale_init/$(printf '%04d' $m) $TMPDAT/exec $TMPDAT \
               $bdy_startframe
        fi
      #------
    ...
    if (pdrun $g $PROC_OPT); then
      if ((BDY_FORMAT == 1)); then
        bash $SCRP_DIR/src/pre_scale.sh $MYRANK $mem_np \
             ...
             $TMPOUT/${stimes[$c]}/topo/topo $TMPOUT/${stimes[$c]}/landuse/landuse \
             ${stimes[$c]} $CYCLEFLEN $CYCLEFLEN $CYCLEFOUT $TMPRUN/scale/$(printf '%04d' $m) $TMPDAT/exec $TMPDAT $time_bdy
      elif ((BDY_FORMAT == 2)); then
        bash $SCRP_DIR/src/pre_scale.sh $MYRANK $mem_np \
             $TMPOUT/${stimes[$c]}/anal/${name_m[$m]}/init $ocean_base $bdy_base \
             $TMPOUT/${stimes[$c]}/topo/topo $TMPOUT/${stimes[$c]}/landuse/landuse \
             ${stimes[$c]} $CYCLEFLEN $CYCLEFLEN $CYCLEFOUT $TMPRUN/scale/$(printf '%04d' $m) $TMPDAT/exec $TMPDAT
#      elif ((BDY_FORMAT == 3)); then
      fi
...
#    fi
    ...
    if (pdrun $g $PROC_OPT); then
      bash $SCRP_DIR/src/post_scale.sh $MYRANK $mem_np \
           ${stimes[$c]} ${name_m[$m]} $CYCLEFLEN $TMPRUN/scale/$(printf '%04d' $m) $LOG_OPT fcst
    fi
  fi
done
