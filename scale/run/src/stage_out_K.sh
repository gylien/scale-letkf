...
function stage_out_K_sub () {
  ...
  if [[ -z "$source" || -z "$destin" ]]; then
    ...
    if ((USE_RANKDIR == 1)) && [[ "$TYPE" == 'share' ]]; then
      echo "#PJM --stgout-dir \"rank=$((i % NRANKS)) $((i % NRANKS)):../$(dirname "$source")/$(basename "$source") $(dirname "$destin")/$(basename "$destin") recursive=${max_depth}\""
    elif ((USE_RANKDIR == 1)) && [[ "$TYPE" == 'local' ]]; then
      if ((n == 0)); then
        echo "#PJM --stgout-dir \"rank=* %r:./$(dirname "$source")/$(basename "$source") $(dirname "$destin")/$(basename "$destin") recursive=${max_depth}\""
      else
        echo "#PJM --stgout-dir \"rank=${i} ${i}:./$(dirname "$source")/$(basename "$source") $(dirname "$destin")/$(basename "$destin") recursive=${max_depth}\""
      fi
    else # ((USE_RANKDIR == 0)) && [[ "$TYPE" == 'share' ]]
      echo "#PJM --stgout-dir \"./$(dirname "$source")/$(basename "$source") $(dirname "$destin")/$(basename "$destin") recursive=${max_depth}\""
    fi
  else
