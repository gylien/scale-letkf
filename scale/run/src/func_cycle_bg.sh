...
pdbash node $PROC_OPT \
  $SCRP_DIR/src/pre_scale_pp_node.sh \
  $mem_nodes $mem_np $TMPRUN/scale_pp $TMPDAT/exec $TMPDAT $MEMBER_RUN 1

for m in $(seq $MEMBER_RUN); do
  echo "  ${timefmt}: node ${node_m[$m]} [$(datetime_now)]"

  ( pdbash proc.${name_m[$m]} $PROC_OPT \
      $SCRP_DIR/src/pre_scale_pp.sh $time \
      $TMPRUN/scale_pp/$(printf '%04d' $m) $TMPDAT/exec $TMPDAT ;
    mpirunf proc.${name_m[$m]} \
      $TMPRUN/scale_pp ./scale-les_pp $(printf '%04d' $m)/pp.conf ;
    pdbash proc.${name_m[$m]} $PROC_OPT \
      $SCRP_DIR/src/post_scale_pp.sh $mem_np $time \
