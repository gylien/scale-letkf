...
!!  END IF
  ...
  var_local(:,10)= VAR_LOCAL_GPR(:) ! GPR
  var_local_n2n(1) = 1
  DO n=2,nv3d+nv2d
    DO i=1,n
      var_local_n2n(n) = i
...
!      WRITE(6,'(A,I6.6,3A,I6.6,A)') 'MYRANK ',myrank,' is writing a file ',NOBS_OUT_BASENAME,'.pe',myrank_d,'.nc'
  ...
  IF(INFL_ADD > 0.0d0) THEN
    ...
    if (INFL_ADD_REF_ONLY) then
      ...
      ic = ctype_elmtyp(uid_obs(id_gpr_obs), 25)
      if (ic > 0) then
        do ij = 1, nij1
          ref_min_dist = 1.0d33
...
!debug okzk
if ( addinfl_weight(ij) > 0.0d0 ) write(*,*) 'INFL_ADD works @', nij1, addinfl_weight(ij)
!debug okzk
        end do
      end if
    else
...
!-----------------------------------------------------------------------
SUBROUTINE calc_qvdef(t,q,p,qvd)
  IMPLICIT NONE
  ...
  REAL(r_size),INTENT(OUT) :: qvd
  REAL(r_size) :: e,es,tc,qs
  ...
  END IF
  qs = (0.622d0 * es)/(p * 0.01d0 - 0.378d0 * es)

  qvd = qs - q

  RETURN
END SUBROUTINE calc_qvdef
!!-----------------------------------------------------------------------
...
#endif
#ifdef GPR
  else if (obtyp == 25) then ! obtypelist(obtyp) == 'GSATPR'                ! GPR
    !nd_v = ABS(LOG(obs(obset)%lev(obidx)) - LOG(rlev)) / vert_loc_ctype(ic) ! GPR
    nd_v = ABS(obs(obset)%lev(obidx) - rz) / vert_loc_ctype(ic)             ! GPR
#endif /* GPR */
  else
