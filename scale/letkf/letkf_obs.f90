...
#ifdef GPR
  real(r_size) :: gpr_threshold
  real(r_size) :: ril, rjl, rk, tmp_save
  real(r_size) :: sig_b
  integer,allocatable :: cnt_all(:,:) 
  integer,allocatable :: cnt_glb(:) 
  integer :: iqc_typ
  integer,allocatable :: cnt_good_rh_d(:)
  integer :: cnt_good_rh
#endif /* GPR */
...
#if defined(GPR) && !defined(GPRfrac)
      elseif (obs(iof)%elm(n) == id_gpr_obs) then
        if (obs(iof)%dat(n) < MIN_GPR_REF_DBZ) then
          obs(iof)%dat(n) = MIN_GPR_REF_DBZ + LOW_REF_SHIFT
        end if
#endif /* GPR && !GPRfrac */
      end if
...
#ifdef GPR
  allocate(cnt_good_rh_d(0:MEM_NP-1))
  cnt_good_rh_d = 0
  cnt_good_rh = 0
#endif /* GPR */
...
!!!###### GPR assimilation ######
    ...
    gpr_threshold = 0.d0
#else
    gpr_threshold = MIN_GPR_REF_DBZ
#endif /* GPRfrac */

    if (obs(iof)%elm(iidx) == id_gpr_obs) then
      if (obs(iof)%dat(iidx) == -999.d0) then
        obsda%qc(n) = iqc_obs_bad
        cycle
      end if
      ...
      mem_ref = 0
      do i = 1, MEMBER
        if (obsda%ensval(i,n) > gpr_threshold) then
        !if (obsda%ensval(i,n) > MIN_GPR_REF_DBZ+1.0d-6) then
          mem_ref = mem_ref + 1
        end if
      end do

      if (obs(iof)%dat(iidx) > gpr_threshold) then
      !if (obs(iof)%dat(iidx) > MIN_GPR_REF_DBZ+1.0d-6) then
        if (mem_ref < MIN_GPR_REF_MEMBER_OBSREF) then
#ifdef GPRmoist
          ! flag
          obs(iof)%idxtmp(iidx) = 1
          ! change obs info
          obs(iof)%dat(iidx) = RH_GPR_VAL   ! assume RH=100% around Z > MIN_GPR_REF_DBZ
          obs(iof)%err(iidx) = RH_GPR_ERR   ! obs err [%]
          ! change Hx
          do i = 1, MEMBER
            obsda%ensval(i,n) = obsda%enstmp(i,n)
          enddo
          ! quality flag
          obsda%qc(n) = iqc_good
!$OMP ATOMIC
          cnt_good_rh_d(myrank_d) = cnt_good_rh_d(myrank_d) + 1
        ...
        if (mem_ref < MIN_GPR_REF_MEMBER) then
          obsda%qc(n) = iqc_ref_no_no
          cycle
        end if
      end if
    endif
!!!###### end GPR assimilation ######
#endif /* GPR */
...
! CA is stored in obsda%val2
    ...
    ! AOEI
    if( GPR_AOEI )then
      !-- compute sprd in obs space
      sig_b = obsda%ensval(1,n) * obsda%ensval(1,n)
      DO i=2,MEMBER
        sig_b = sig_b + obsda%ensval(i,n) * obsda%ensval(i,n)
      ENDDO
      sig_b = sig_b / sqrt(REAL(MEMBER,r_size))
      !-- obs error infl
      obs(iof)%errtmp(iidx) = sqrt(max(obs(iof)%err(iidx)**2,obsda%val(n)**2-sig_b))
      obs(iof)%err(iidx) = sqrt(max(obs(iof)%err(iidx)**2,obsda%val(n)**2-sig_b))
    endif
#endif /* AOEI */
...
!      END IF
    ...
    case (id_tcmip_obs)
      ...
      END IF
    case (id_gpr_obs)
      IF(ABS(obsda%val(n)) > GROSS_ERROR_GPR * obs(iof)%err(iidx)) THEN
        obsda%qc(n) = iqc_gross_err
      END IF
    case default
...
#ifdef GPRmoist
  do iof = 1, OBS_IN_NUM
    call MPI_ALLREDUCE(MPI_IN_PLACE, obs(iof)%idxtmp, obs(iof)%nobs, MPI_INTEGER, MPI_SUM, MPI_COMM_a, ierr)
    !call MPI_ALLREDUCE(MPI_IN_PLACE, obs(iof)%idxtmp, obs(iof)%nobs, MPI_INTEGER, MPI_SUM, MPI_COMM_d, ierr)
    do n = 1, obs(iof)%nobs
      if( obs(iof)%idxtmp(n) >= 1 )then
        obs(iof)%dat(n) = RH_GPR_VAL    ! assume RH=100% around Z > MIN_GPR_REF_DBZ
        obs(iof)%err(n) = RH_GPR_ERR    ! obs err [%]
      endif
    enddo ! [ n = 1, obs(iof)%nobs ]
  enddo ! [ iof = 1, OBS_IN_NUM ]
#endif /* GPRmoist */

  if( GPR_AOEI )then
    do iof = 1, OBS_IN_NUM
      call MPI_ALLREDUCE(MPI_IN_PLACE, obs(iof)%errtmp, obs(iof)%nobs, MPI_r_size, MPI_MAX, MPI_COMM_a, ierr)
      do n = 1, obs(iof)%nobs
        if( obs(iof)%errtmp(n) .gt. 0.d0 ) obs(iof)%err(n) = obs(iof)%errtmp(n)
      enddo ! [ n = 1, obs(iof)%nobs ]
    enddo ! [ iof = 1, OBS_IN_NUM ]
  endif
#endif /* GPR */
...
!-------------------------------------------------------------------------------
  ...
  do n = 1, obsda%nobs
    ...
    IF(obs(iof)%elm(iidx) == id_gpr_obs)THEN
      ...
      cnt_all(1,myrank_d) = cnt_all(1,myrank_d) + 1
      if( obsda%qc(n) == iqc_good      ) cnt_all(2 ,myrank_d)  = cnt_all(2 ,myrank_d) + 1
      if( obsda%qc(n) == iqc_gross_err ) cnt_all(3 ,myrank_d)  = cnt_all(3 ,myrank_d) + 1
      if( obsda%qc(n) == iqc_ps_ter    ) cnt_all(4 ,myrank_d)  = cnt_all(4 ,myrank_d) + 1
      if( obsda%qc(n) == iqc_ref_low   ) cnt_all(5 ,myrank_d)  = cnt_all(5 ,myrank_d) + 1
      if( obsda%qc(n) == iqc_ref_mem   ) cnt_all(6 ,myrank_d)  = cnt_all(6 ,myrank_d) + 1
      if( obsda%qc(n) == iqc_radar_vhi ) cnt_all(7 ,myrank_d)  = cnt_all(7 ,myrank_d) + 1
      if( obsda%qc(n) == iqc_out_vhi   ) cnt_all(8 ,myrank_d)  = cnt_all(8 ,myrank_d) + 1
      if( obsda%qc(n) == iqc_out_vlo   ) cnt_all(9 ,myrank_d)  = cnt_all(9 ,myrank_d) + 1
      if( obsda%qc(n) == iqc_obs_bad   ) cnt_all(10,myrank_d)  = cnt_all(10,myrank_d) + 1
      if( obsda%qc(n) == iqc_otype     ) cnt_all(11,myrank_d)  = cnt_all(11,myrank_d) + 1
      if( obsda%qc(n) == iqc_time      ) cnt_all(12,myrank_d)  = cnt_all(12,myrank_d) + 1
  ...
  call MPI_ALLREDUCE(cnt_good_rh_d(myrank_d), cnt_good_rh, 1, MPI_INTEGER, MPI_SUM, MPI_COMM_d, ierr)
  do iqc_typ = 1, 15
    call MPI_ALLREDUCE(cnt_all(iqc_typ,myrank_d), cnt_glb(iqc_typ), 1, MPI_INTEGER, MPI_SUM, MPI_COMM_d, ierr)
  enddo
  ...
  write(*,*) ' (Conv. to RH) ',cnt_good_rh
  write(*,*) ' IQC_GROSS_ERR ',cnt_glb( 3)
  write(*,*) ' IQC_PS_TER    ',cnt_glb( 4)
  write(*,*) ' IQC_REF_LOS   ',cnt_glb( 5)
  write(*,*) ' IQC_REF_MEM   ',cnt_glb( 6)
  write(*,*) ' IQC_REF_NO_NO ',cnt_glb(15)
  write(*,*) ' IQC_RADAR_VHI ',cnt_glb( 7)
  write(*,*) ' IQC_OUT_VHI   ',cnt_glb( 8)
  write(*,*) ' IQC_OUT_VLO   ',cnt_glb( 9)
  write(*,*) ' IQC_OBS_BAD   ',cnt_glb(10)
  write(*,*) ' IQC_OTYPE     ',cnt_glb(11)
