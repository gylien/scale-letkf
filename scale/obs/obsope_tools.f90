...
#ifdef GPR
  REAL(r_size),ALLOCATABLE :: v1dg(:,:)
  REAL(r_size),ALLOCATABLE :: ri_gpr(:),rj_gpr(:),rk_gpr(:)
  real(r_size),allocatable :: rlon_gpr(:),rlat_gpr(:),rlev_gpr(:)
  INTEGER,ALLOCATABLE :: elm_gpr(:)
  INTEGER :: nobs_gpr
#endif
...
#ifdef GPR
        call read_ens_history_iter_1d(1,islot,v1dg)
#endif
...
#ifdef GPR
              !! GPR obs will be precessed separately
              if( obs(iof)%elm(n) == id_gpr_obs ) cycle
#endif /* GPR */
...
!   -- End of TC vital DA -- 
...
#ifdef GPR
        allocate( ri_gpr  (n1:n2) )
        allocate( rj_gpr  (n1:n2) )
        allocate( rk_gpr  (n1:n2) )
        allocate( elm_gpr (n1:n2) )
        allocate( rlon_gpr(n1:n2) )
        allocate( rlat_gpr(n1:n2) )
        allocate( rlev_gpr(n1:n2) )
        ...
        do nn = n1, n2
          ...
          rj_gpr(nn) = rjl
          rk_gpr(nn) = rk
          rlon_gpr(nn) = obs(iof)%lon(n)
          rlat_gpr(nn) = obs(iof)%lat(n)
          rlev_gpr(nn) = obs(iof)%lev(n) ! (m)

          elm_gpr(nn) = obs(iof)%elm(n) ! copy for Trans_XtoY_GPR

        enddo

        if( nobs_gpr > 0 )then
          CALL Trans_XtoY_GPR(n1,n2,elm_gpr(n1:n2),ri_gpr(n1:n2),rj_gpr(n1:n2),rk_gpr(n1:n2),rlon_gpr(n1:n2),rlat_gpr(n1:n2),rlev_gpr(n1:n2),v3dg,v2dg,v1dg,obsda%val(n1:n2),obsda%tmp(n1:n2),obsda%qc(n1:n2),MPI_COMM_d)
        endif

        deallocate( ri_gpr, rj_gpr, rk_gpr, elm_gpr, rlon_gpr, rlat_gpr, rlev_gpr )
#endif /* GPR */
...
#ifdef GPR
        if (im == mmdetin) then
          obsda%enstmp(mmdetobs, 1:nobs) = obsda%tmp(1:nobs)
        else
          obsda%enstmp(im, 1:nobs) = obsda%tmp(1:nobs)
        end if
#endif /* GPR */
...
#ifdef GPR
    if (nprocs_e > 1) then
      call MPI_ALLREDUCE(MPI_IN_PLACE, obsda%enstmp(:,1:nobs), nensobs*nobs, MPI_r_size, MPI_SUM, MPI_COMM_e, ierr)
    end if
#endif /* GPR */
...
!-----------------------------------------------------------------------
  ...
  do islot = SLOT_START, SLOT_END
    ...
    do iof = 1, OBS_IN_NUM
      IF(OBS_IN_FORMAT(iof) <= 2)THEN ! except H08 & GPR
        nslot = 0
        nobs_slot = 0
        do n = 1, obs(iof)%nobs
...
#endif
!#ifdef GPR
!      ELSEIF( OBS_IN_FORMAT(iof) == 4 )THEN
!        nslot = 0
!        nobs_slot = 0
!        do n = 1, obs(iof)%nobs
!
...
!        enddo
!
!        CALL Trans_XtoY_GPR(obs(iof),obsda,nobs_0,nobs,nobs_alldomain,v3dg,v2dg,v1dg,ri(1:nobs),rj(1:nobs))
!
!#endif /* GPR */
      ENDIF
...
!-------------------------------------------------------------------------------
#ifdef GPR
subroutine obssim_cal(v3dgh, v2dgh, v1dgh, v3dgsim, v2dgsim, stggrd)
#else
...
!debug
  ...
  integer :: tmpqc
#ifdef GPR
  real(r_size), intent(in) :: v1dgh(nlevh,nv1dd)
  integer :: n1,n2,n
  real(r_size),allocatable :: tmpobs_gpr(:), tmpobs_rh(:)
  real(r_size),allocatable :: ri_gpr(:),rj_gpr(:),rk_gpr(:)
  real(r_size),allocatable :: rlon_gpr(:),rlat_gpr(:),rlev_gpr(:)
  integer,allocatable :: tmpqc_gpr(:)
  integer,allocatable :: elm_gpr(:)
      ...
      do j = 1, nlat
        ...
        do i = 1, nlon
          ...
          do k = 1, nlev
            ...
            rj_gpr(n) = rj
            rk_gpr(n) = rk
            elm_gpr(n) = id_gpr_obs

            call MPRJ_xy2lonlat((ri-1.0d0) * DX + GRID_CX(1), (rj-1.0d0) * DY + GRID_CY(1), lon, lat)
            rlon_gpr(n) = lon * rad2deg
            rlat_gpr(n) = lat * rad2deg
            rlev_gpr(n) = v3dgh(k+KHALO, i+IHALO, j+JHALO, iv3dd_hgt)
