...
!-------------------------------------------------------------------------------
#ifdef GPR
subroutine read_ens_history_iter_1d(iter, step, v1dg)
  implicit none
  integer, intent(in) :: iter
  integer, intent(in) :: step
  ...
  if (im >= 1 .and. im <= nens) then
    ...
    call read_history_1d(trim(filename), step, v1dg)
  end if

  return
end subroutine read_ens_history_iter_1d
#endif /* GPR */
...
#ifdef GPR
    call MPI_BCAST(obs(iof)%idxtmp, obs(iof)%nobs, MPI_INTEGER, 0, MPI_COMM_a, ierr)
    call MPI_BCAST(obs(iof)%errtmp, obs(iof)%nobs, MPI_r_size, 0, MPI_COMM_a, ierr)
#endif
