...
!    I_QV, I_HC, I_HR, I_HI, I_HS, I_HG
  ...
  if ( OUT_NETCDF_DAFCST ) then
    ...
    endif ! step == 0 .or. mod( step, OUT_NETCDF_DAFCST_DSTEP) == 0 

  endif ! OUT_NETCDF_DAFCST


  if ( OUT_GRADS_DAFCST ) then

    nlev_ = 0
    do k = 1, nlev
      if ( CZ(KHALO+k) > OUT_GRADS_DAFCST_ZLEV ) exit
      nlev_ = k
    enddo

    if ( myrank_d == 0 ) then
      filename = trim(DACYCLE_RUN_FCST_OUTNAME)//"/"//trim(timelabel)//".grd"
      iunit = 55
      ...
      irec = step * nlev_ * 1 ! 1 variable (nlev*1 record) output 
    end if
