...
!$USE OMP_LIB
  ...
  INTEGER,PARAMETER :: nid_obs_varlocal=10 !H08, GPR
!
...
!
  ...
  CHARACTER(3),PARAMETER :: obelmlist_varlocal(nid_obs_varlocal)= &
     (/'WND', '  T', 'MOI', ' PS', 'PRC', 'TCV', 'REF', ' Vr', 'H08', 'GPR'/)
  ...
  CHARACTER(6),PARAMETER :: obtypelist(nobtype)= &
     (/'ADPUPA', 'AIRCAR', 'AIRCFT', 'SATWND', 'PROFLR', &
       ...
       'TMPAPR', 'PHARAD', 'H08IRB', 'TCVITL', 'GSATPR'/) ! H08, GPR
  ...
  TYPE obs_info
    ...
    REAL(r_size),ALLOCATABLE :: errtmp(:)
#endif /* GPR */
  END TYPE obs_info
...
#endif
#ifdef GPR
    REAL(r_size),ALLOCATABLE :: tmp(:) ! GPR
    REAL(r_size),ALLOCATABLE :: enstmp(:,:) ! GPR
#endif /* GPR */
  ...
  INTEGER,PARAMETER :: nobsformats=4 ! H08, GPR
  CHARACTER(30),PARAMETER :: obsformat_name(nobsformats) = &
    (/'CONVENTIONAL ', 'RADAR        ', 'Himawari-8-IR', 'GeoSat-PR    '/)
...
#ifdef GPR
  ALLOCATE( obs%idxtmp(obs%nobs) )
  ALLOCATE( obs%errtmp(obs%nobs) )
#endif /* GPR */
...
#ifdef GPR
  IF(ALLOCATED(obs%idxtmp)) DEALLOCATE(obs%idxtmp)
  IF(ALLOCATED(obs%errtmp)) DEALLOCATE(obs%errtmp)
#endif /* GPR */
...
SUBROUTINE obs_da_value_allocate(obsda,member)
...
#ifdef GPR
  ALLOCATE( obsda%tmp (obsda%nobs) ) ! GPR
#endif /* GPR */
  ...
  if (member > 0) then
    ...
    obsda%ensval = 0.0d0
#ifdef GPR
    ALLOCATE( obsda%enstmp (member,obsda%nobs) )
    obsda%enstmp = 0.0d0
#endif /* GPR */
  end if
...
#ifdef GPR
  IF(ALLOCATED(obsda%tmp   )) DEALLOCATE(obsda%tmp   ) ! GPR
  IF(ALLOCATED(obsda%enstmp)) DEALLOCATE(obsda%enstmp) ! GPR
#endif /* GPR */
...
subroutine read_obs_all(obs)
  ...
  do iof = 1, OBS_IN_NUM
    ...
    case (4) !GPR
      call get_nobs_GPR(trim(OBS_IN_NAME(iof)),obs(iof)%nobs) ! GPR
    case default
    ...
    case (4) ! GPR
      call read_obs_GPR(trim(OBS_IN_NAME(iof)),obs(iof)) ! GPR
    end select
...
subroutine write_obs_all(obs, missing, file_suffix)
  ...
  do iof = 1, OBS_IN_NUM
    ...
    case (4) ! GPR
      call write_obs_GPR(trim(filestr),obs(iof),missing=missing_) ! GPR
    end select
  end do ! [ iof = 1, OBS_IN_NUM ]
...
#ifdef GPR
!-----------------------------------------------------------------------
! Precipitation radar measurement from space (Ku-band)
! (17/03/07) A.Okazaki 
!-----------------------------------------------------------------------
SUBROUTINE Trans_XtoY_GPR(n1,n2,elm,ri,rj,rk,rlon,rlat,rlev,v3d,v2d,v1d,yobs,yobs_tmp,qc,MPI_COMM_d)
  use common_mpi
  use scale_grid_index, only: &
      IHALO,JHALO,KHALO
  use common_nml, only: &
  ...
  INTEGER,INTENT(IN) :: elm(n1:n2)
  REAL(r_size),INTENT(IN) :: ri(n1:n2),rj(n1:n2),rk(n1:n2)
  REAL(r_size),INTENT(IN) :: rlon(n1:n2),rlat(n1:n2),rlev(n1:n2)
  REAL(r_size),INTENT(IN) :: v3d(nlevh,nlonh,nlath,nv3dd)
  REAL(r_size),INTENT(IN) :: v2d(nlonh,nlath,nv2dd)
  REAL(r_size),INTENT(IN) :: v1d(nlevh,nv1dd)
  REAL(r_size),INTENT(OUT) :: yobs(n1:n2)
  REAL(r_size),INTENT(OUT) :: yobs_tmp(n1:n2)   ! relative humidity
...
! relative humidity
  ...
  integer,parameter :: n_avrg = (2*ij_avrg+1)**2
  integer :: i0,j0,k0
...
!write(*,*) 'dr',dr
  ...
  ! set the lowest layer number that has a defined values
  !!!surface%k1layer=1
  !!!k1=surface%k1layer
  k1=1+KHALO

  allocate(hgt_lay(nlevh), &
           hgt_lev(nlevh))
  allocate(topo   (nlonh,nlath) )
  allocate(atmos  (nlevh,nlonh,nlath), &
           rho    (nlevh,nlonh,nlath), &
           atmos_stag(nlevh,nlonh,nlath))

  !++ height (tracer point) [m]
  hgt_lay(:) = v1d(:,iv1dd_lay)
  !++ height (staggered point) [m]
  hgt_lev(:) = v1d(:,iv1dd_lev)
  hgt_lev(ks-1) = 0.d0
  !++ surface elevation [m]
  topo(:,:) = v2d(:,:,iv2dd_topo)
  do j=js,je ; do i=is,ie ; do k=ks,ke
    atmos(k,i,j)%hgt = ( topo(i,j) &
                     + ( hgt_lev(ke)-topo(i,j))/hgt_lev(ke) * hgt_lay(k) ) &
                     * 0.001d0 ! [m] --> [km]
    atmos_stag(k,i,j)%hgt = ( topo(i,j) &
                     + ( hgt_lev(ke)-topo(i,j))/hgt_lev(ke) * hgt_lev(k) ) &
                     * 0.001d0 ! [m] --> [km]
  enddo ; enddo ; enddo
  !++ 2m air temp [K]
  do j=js,je ; do i=is,ie
    atmos_stag(k1-1,i,j)%t_air = v2d(i,j,iv2dd_t2m)
  enddo ; enddo
  !++ temperature at staggered point
  do j=js,je ; do i=is,ie ; do k=k1,ke-1
    atmos_stag(k,i,j)%t_air = (v3d(k+1,i,j,iv3dd_t)-v3d(k,i,j,iv3dd_t)) &
                            / (atmos(k+1,i,j)%hgt-atmos(k,i,j)%hgt) &
                            * (atmos_stag(k,i,j)%hgt-atmos(k,i,j)%hgt)+v3d(k,i,j,iv3dd_t)
                            !* (atmos_stag(k,i,j)%hgt-atmos(k,i,j)%hgt)+atmos(k,i,j)%t_air
  enddo ; enddo ; enddo
  do j=js,je ; do i=is,ie
    atmos_stag(ke,i,j)%t_air = (v3d(ke,i,j,iv3dd_t)-v3d(ke-1,i,j,iv3dd_t)) &
                            / (atmos(ke,i,j)%hgt-atmos(ke-1,i,j)%hgt) &
                            * (atmos_stag(ke,i,j)%hgt-atmos(ke,i,j)%hgt)+v3d(ke,i,j,iv3dd_t)
                            !* (atmos_stag(ke,i,j)%hgt-atmos(ke,i,j)%hgt)+atmos(ke,i,j)%t_air
  enddo ; enddo
  !++ convert pressur unit (Pa --> hPa)
  !atmos(:,:,:)%press = atmos(:,:,:)%press * 0.01
  !++ versitcal wind [m/s]
  atmos(:,:,:)%omega = v3d(:,:,:,iv3dd_w)
  do j=js,je ; do i=is,ie ; do k=ks,ke-1
    w2=atmos(k+1,i,j)%hgt-atmos_stag(k,i,j)%hgt
    w1=atmos_stag(k,i,j)%hgt-atmos(k,i,j)%hgt
    atmos_stag(k,i,j)%omega = (w2*atmos(k,i,j)%omega + w1*atmos(k+1,i,j)%omega)/(w1+w2)
  enddo ; enddo ; enddo
  do j = js, je ; do i = is, ie
...
!!< ONCE
!!<-----------------------------------------------------------------------
  call bulk_dsd   ! DSD for exponential DSDs
!< call opt_micro_radar
  mxfreq = 1
  nf = mxfreq
  freqcy = freq_radar
...
!!<-----------------------------------------------------------------------
  do j=js,je ; do i=is,ie ; do k=ks,ke
      ...
      ! 3D --> scaler
      q_gce_s%cloud  = v3d(k,i,j,iv3dd_qc)*v3d(k,i,j,iv3dd_rho)*1000.d0 ! QC [kg/kg] --> [g/m3] 
      q_gce_s%rain   = v3d(k,i,j,iv3dd_qr)*v3d(k,i,j,iv3dd_rho)*1000.d0 ! QR [kg/kg] --> [g/m3]
      q_gce_s%snow   = v3d(k,i,j,iv3dd_qs)*v3d(k,i,j,iv3dd_rho)*1000.d0 ! QS [kg/kg] --> [g/m3]
      q_gce_s%graupel= v3d(k,i,j,iv3dd_qg)*v3d(k,i,j,iv3dd_rho)*1000.d0 ! QG [kg/kg] --> [g/m3]
      q_gce_s%ice    = v3d(k,i,j,iv3dd_qi)*v3d(k,i,j,iv3dd_rho)*1000.d0 ! QI [kg/kg] --> [g/m3]
      q_gce_s%hail   = 0.d0
      atmos_s%t_air  = v3d(k,i,j,iv3dd_t)
      atmos_s%press  = v3d(k,i,j,iv3dd_p) * 0.01 ! [Pa] --> [hPa]
      atmos_s%rh     = v3d(k,i,j,iv3dd_rh)
      ...
      if( atmos_stag(k-1,i,j)%t_air.ge.const_tmelt.and.atmos_stag(k,i,j)%t_air.le.const_tmelt )then
        melt_frac_bulk = ( atmos_stag(k-1,i,j)%t_air - const_tmelt ) / &
                         ( atmos_stag(k-1,i,j)%t_air - atmos_stag(k,i,j)%t_air )
      elseif( atmos_stag(k-1,i,j)%t_air.lt.const_tmelt )then
        melt_frac_bulk = 0.d0
      elseif( atmos_stag(k,i,j)%t_air.gt.const_tmelt )then
        melt_frac_bulk = 0.9d0
      endif

      ! exner function
      atmos_s%exner = (atmos_s%press * 0.001d0)**(const_Rd/1004.d0)
      ! layer thickness [km]
      atmos_s%dhgt = atmos_stag(k,i,j)%hgt-atmos_stag(k-1,i,j)%hgt
      ! layer height [km]
      atmos_s%hgt = 0.5d0 * (atmos_stag(k,i,j)%hgt+atmos_stag(k-1,i,j)%hgt)
      ! vertical velocity [m/s]
      atmos_s%omega = 0.5d0 * (atmos_stag(k,i,j)%omega+atmos_stag(k-1,i,j)%omega)

!!<-----------------------------------------------------------------------
!!< call re_all     ! compute effective radius drop size
      call re_bulk_mono(0, rho_gce%cloud  ,n0_gce%cloud  ,q_gce_s%cloud  ,re_gce_s%cloud )
      call re_LUT_Heymsfield_Platt_1984('proc', atmos_s%t_air, q_gce_s%ice, re_gce_s%ice )
      call re_bulk_exp(    rho_gce%rain   ,n0_gce%rain   ,q_gce_s%rain   ,re_gce_s%rain)
      call re_bulk_exp(    rho_gce%snow   ,n0_gce%snow   ,q_gce_s%snow   ,re_gce_s%snow)
      call re_bulk_exp(    rho_gce%graupel,n0_gce%graupel,q_gce_s%graupel,re_gce_s%graupel)
      call re_bulk_exp(    rho_gce%hail   ,n0_gce%hail   ,q_gce_s%hail   ,re_gce_s%hail)

!!<-----------------------------------------------------------------------
!!< call opt_micro_radar
      ! Gas absorption
      call gas_absorb( freqcy, tavg, pres, atmos_s%rh, 0.0_r_size, atm_ext )
      ! Condensate particle optical properties
      call opt_gce_micro( nf, freqcy, tavg, melt_frac_bulk, q_gce_s, re_gce_s, &
      ...
      call total_opt_gce( atm_ext, kext_gce, salb_gce, asym_gce, pbck_gce, &
                          kexttot_s, salbtot_s, asymtot_s, sbacktot_s )

      sbacktot_l(k,i,j) = sbacktot_s
...
#if defined(GPRsatellite) || defined(GPRavrg)
  call MPI_BARRIER(MPI_COMM_d,ierr)
  call rank_1d_2d(PRC_myrank,proc_i,proc_j)
...
if(myrank == 0) write(*,'(A,4E20.5)') '+',pr,sb,w,dv
 ...
 1113 continue
      ...
      k0 = CEILING(rk(nn))-KHALO

      if( i0 - ij_avrg < 1 .or. i0 + ij_avrg > nlong .or. &
          j0 - ij_avrg < 1 .or. j0 + ij_avrg > nlatg ) then
        yobs(nn) = undef
        qc(nn) = iqc_out_h
        cycle
      else
        sbacktot_s = 0.d0
        do i=i0-ij_avrg,i0+ij_avrg ; do j=j0-ij_avrg,j0+ij_avrg ! horizontal average
          sbacktot_s = sbacktot_s + sbacktot_g(k0,i,j)
        enddo ; enddo
      ...
      if( sbacktot_s <= 0.d0 )then
        ...
        dbz = 10.d0*(4.d0*log10(2.997925d2/freq_radar) + &
                        log10(sbacktot_s) - &
...
!-----------------------------------------------------------------------
SUBROUTINE itpl_2d_g(var,ri,rj,var5)
  IMPLICIT NONE
  REAL(r_size),INTENT(IN) :: var(nlong,nlatg)
  REAL(r_size),INTENT(IN) :: ri
  REAL(r_size),INTENT(IN) :: rj
  REAL(r_size),INTENT(OUT) :: var5
  REAL(r_size) :: ai,aj
  ...
  RETURN
END SUBROUTINE itpl_2d_g

SUBROUTINE itpl_2d_column_g(var,ri,rj,var5)
  IMPLICIT NONE
  REAL(r_size),INTENT(IN) :: var(nlevh,nlong,nlatg)
  REAL(r_size),INTENT(IN) :: ri
  REAL(r_size),INTENT(IN) :: rj
  REAL(r_size),INTENT(OUT) :: var5(nlevh)
  REAL(r_size) :: ai,aj
  ...
  RETURN
END SUBROUTINE itpl_2d_column_g

SUBROUTINE itpl_3d_g(var,ri,rj,rk,var5)
  IMPLICIT NONE
  REAL(r_size),INTENT(IN) :: var(nlev,nlong,nlatg)
  REAL(r_size),INTENT(IN) :: ri
  REAL(r_size),INTENT(IN) :: rj
  REAL(r_size),INTENT(IN) :: rk
  REAL(r_size),INTENT(OUT) :: var5
  ...
  RETURN
END SUBROUTINE itpl_3d_g

SUBROUTINE phys2ijkz_g(z_full,ri,rj,rlev,rk,qc)
  use scale_grid_index, only: &
      KHALO
  IMPLICIT NONE

  REAL(r_size),INTENT(IN) :: z_full(nlevh,nlong,nlatg)
  REAL(r_size),INTENT(IN) :: ri
  REAL(r_size),INTENT(IN) :: rj
  REAL(r_size),INTENT(IN) :: rlev ! height levels
  REAL(r_size),INTENT(OUT) :: rk
...
!
  ...
  call itpl_2d_column_g(z_full,ri,rj,zlev)

  !
  ! determine if rlev is within bound.
  !
  IF(rlev > zlev(nlev+KHALO)) THEN
    call itpl_2d_g(z_full(nlev+KHALO,:,:),ri,rj,ztmp)
    !write(6,'(A,F8.1,A,F8.1)') 'warning: observation is too high: ztop=', ztmp, ', lev=', rlev
    rk = undef
    qc = iqc_out_vhi
    RETURN
...
SUBROUTINE get_nobs_GPR(cfile,nn)
  IMPLICIT NONE
  CHARACTER(*),INTENT(IN) :: cfile
  INTEGER,INTENT(OUT) :: nn
...
!    WRITE(6,'(A12,I10)') '   TC TRACK:',itc
  ...
  RETURN
END SUBROUTINE get_nobs_GPR

SUBROUTINE read_obs_GPR(cfile,obs)
  use scale_mapproj, only: &
      MPRJ_lonlat2xy
  IMPLICIT NONE
  CHARACTER(*),INTENT(IN) :: cfile
...
!  call obs_info_allocate(obs)
  ...
  DO n=1,obs%nobs
    ...
    obs%lon(n) = REAL(wk(2),r_size)
    obs%lat(n) = REAL(wk(3),r_size)
    obs%lev(n) = REAL(wk(4),r_size)     ! [Pa]
    obs%dat(n) = REAL(wk(5),r_size)     ! [dBZ]
    obs%err(n) = REAL(wk(6),r_size)
    obs%typ(n) = NINT(wk(7))
    obs%dif(n) = REAL(wk(8),r_size)
!tmp
...
!tmp
  ...
  RETURN
END SUBROUTINE read_obs_GPR

SUBROUTINE write_obs_GPR(cfile,obs,append,missing)
  IMPLICIT NONE
  CHARACTER(*),INTENT(IN) :: cfile
  TYPE(obs_info),INTENT(IN) :: obs
  LOGICAL,INTENT(IN),OPTIONAL :: append
