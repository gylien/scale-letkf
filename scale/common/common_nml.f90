...
!===============================================================================
...
#ifdef GPR
  integer, parameter :: nv1d = 2     ! number of 1D state variables (in SCALE restart files)
#endif
  ...
  real(r_size) :: GROSS_ERROR_GPR = -1.0d0       ! < 0: same as GROSS_ERROR
...
!      -1.0d0, 1000.0d0, -1.0d0,  0.0d0/)
  ...
  real(r_size) :: HORI_LOCAL_GPR_OBSNOREF = -1.0d0   ! <0: same as VERT_LOCAL(22=PHARAD)
  ...
  real(r_size) :: OBS_MIN_SPACING(nobtype) = &
    (/300.0d3, 100.0d3, 100.0d3, 150.0d3, 300.0d3, 150.0d3, 150.0d3, 100.0d3, 150.0d3, 150.0d3, &
      ...
      150.0d3,   1.0d3,  15.0d3,1000.0d3, 300.0d3/)
  ...
  !--- PARAM_LETKF_GPR
  INTEGER :: MIN_GPR_REF_MEMBER = 1         !Ensemble members with reflectivity greather than RADAR_REF_THRES_DBZ
  INTEGER :: MIN_GPR_REF_MEMBER_OBSREF = 1  !Ensemble members with
  REAL(r_size) :: MIN_GPR_REF_DBZ = 0.d0    !Minimum reflectivity
  REAL(r_size) :: LOW_GPR_REF_SHIFT = 0.0d0
  REAL(r_size) :: RH_GPR_VAL                !RH for moisturization (%)
  REAL(r_size) :: RH_GPR_ERR                !Observation error of RH for moisturization (%)
  LOGICAL :: GPR_AOEI = .false.             
  ...
  real(r_size) :: OBSERR_GPR = 5.0d0 ! (dBZ)
  real(r_size) :: OBSERR_GPR_NOREF = 20.d0 ! (dBZ)
...
subroutine read_nml_letkf
  ...
  if (GROSS_ERROR_GPR < 0.0d0) then
    GROSS_ERROR_GPR = GROSS_ERROR
  end if
...
!    stop
  ...
  if (HORI_LOCAL_GPR_OBSNOREF < 0.0d0) then
    HORI_LOCAL_GPR_OBSNOREF = HORI_LOCAL(22) !PHARAD
  end if
...
! PARAM_LETKF_GPR
!-------------------------------------------------------------------------------
subroutine read_nml_letkf_gpr
  implicit none
  integer :: ierr

  namelist /PARAM_LETKF_GPR/ &
    MIN_GPR_REF_MEMBER, &
    MIN_GPR_REF_MEMBER_OBSREF, &
    MIN_GPR_REF_DBZ, &
    LOW_GPR_REF_SHIFT, &
    RH_GPR_VAL, &
    RH_GPR_ERR, &
  ...
  rewind(IO_FID_CONF)
  read(IO_FID_CONF,nml=PARAM_LETKF_GPR,iostat=ierr)
  if (ierr < 0) then !--- missing
    write(6,*) 'Warning: /PARAM_LETKF_GPR/ is not found in namelist.'
!    stop
  elseif (ierr > 0) then !--- fatal error
    write(6,*) 'xxx Not appropriate names in namelist PARAM_LETKF_GPR. Check!'
    stop
  endif

  write(6, nml=PARAM_LETKF_GPR)

  return
end subroutine read_nml_letkf_gpr
