...
!-------------------------------------------------------------------------------
...
#ifdef GPR
  INTEGER,PARAMETER :: iv1d_lay=1  !
  INTEGER,PARAMETER :: iv1d_lev=2  !
  CHARACTER(vname_max),PARAMETER :: v1d_name(nv1d) = &
     (/'z', 'zh'/)
#endif
  ...
  ! 
#ifdef GPR
  INTEGER,PARAMETER :: nv3dd=14  ! 3D diagnostic variables (in SCALE history files)
#else
  INTEGER,PARAMETER :: nv3dd=13
#endif /* GPR */
#if defined(H08) || defined(GPR)
  INTEGER,PARAMETER :: nv2dd=9  ! H08
#else
  INTEGER,PARAMETER :: nv2dd=7
#endif /* H08 || GPR */
#ifdef GPR
  INTEGER,PARAMETER :: nv1dd=2   ! 1D diagnostic variables (in SCALE history files)
#endif
...
#ifdef GPR
  INTEGER,PARAMETER :: iv3dd_rho=14
#endif /* GPR */
...
#endif /* H08 || GPR */
#ifdef GPR
  INTEGER,PARAMETER :: iv1dd_lay=1  ! GPR
  INTEGER,PARAMETER :: iv1dd_lev=2  ! GPR
#endif /* GPR */
#ifdef GPR
  CHARACTER(vname_max),PARAMETER :: v3dd_name(nv3dd) = &            ! GPR
     (/'U', 'V', 'W', 'T', 'PRES', &                                ! GPR
       'QV', 'QC', 'QR', 'QI', 'QS', 'QG', 'RH', 'height', 'DENS'/) ! GPR
#else
...
#endif /* H08 || GPR */
#ifdef GPR
  CHARACTER(vname_max),PARAMETER :: v1dd_name(nv1dd) = &
     (/'z', 'zh'/)
#endif /* GPR */
...
#ifdef GPR
SUBROUTINE read_restart_1d(filename,v1dg)
  use netcdf, only: NF90_NOWRITE
  use scale_process, only: &
    PRC_myrank
  use scale_grid_index, only: &
  ...
  CHARACTER(*),INTENT(IN) :: filename
  REAL(RP),INTENT(OUT) :: v1dg(nlev,nv1d)
  character(len=12) :: filesuffix = '.pe000000.nc'
  integer :: iv1d,ncid
  real(RP) :: v1dgtmp(KMAX)
  ...
  do iv1d = 1, nv1d
    write(6,'(1x,A,A15)') '*** Read 1D var: ', trim(v1d_name(iv1d))
    call ncio_read(ncid, trim(v1d_name(iv1d)), KMAX, 1, v1dgtmp)
    v1dg(:,iv1d) = v1dgtmp(:)
  end do
...
!-------------------------------------------------------------------------------
! [File I/O] Read SCALE history files (1D)
!-------------------------------------------------------------------------------
#ifdef GPR
subroutine read_history_1d(filename,step,v1dg)
  use scale_process, only: &
      PRC_myrank
  use scale_grid_index, only: &
      IHALO, JHALO, KHALO, &
  ...
  character(*),intent(in) :: filename
  integer,intent(in) :: step
  real(r_size),intent(out) :: v1dg(nlevh,nv1dd)
  integer :: i,j,k,iv3d,iv2d,iv1d
  character(len=12) :: filesuffix = '.pe000000.nc'
  real(RP) :: var1d(nlev)
  ...
  !-------------
  do iv1d = 1, nv1dd
    write(6,'(1x,A,A15)') '*** Read 1D var: ', trim(v1dd_name(iv1d))
    call HistoryGet( var1D,                 & ! [OUT]
                     filename,              & ! [IN]
                     trim(v1dd_name(iv1d)), & ! [IN]
                     step                   ) ! [IN]
    v1dg(1+KHALO:nlev+KHALO,iv1d) = var1D(:)
  enddo

  !do iv1d = 1, nv1dd
  !  call COMM_vars8( v1dg(:,iv1d), iv1d )
  !end do
  !do iv1d = 1, nv2dd
  !  call COMM_wait ( v2dg(:,iv1d), iv1d )
  !end do

  return
end subroutine read_history_1d
#endif /* GPR */
...
!  v3dgh(1+KHALO:nlev+KHALO,1+IHALO:nlon+IHALO,1+JHALO:nlat+JHALO,iv3dd_rh) = [[RH calculator]]
  do j = 1, nlat
    do i = 1, nlon
      do k = 1, nlev
        call calc_rh_tmp(v3dg(k,i,j,iv3d_t),v3dg(k,i,j,iv3d_q),v3dg(k,i,j,iv3d_p),v3dgh(KHALO+k,IHALO+i,JHALO+j,iv3dd_rh))
      enddo
    enddo
  enddo
  ...
  ! RHO
  !---------------------------------------------------------
  v3dgh(1+KHALO:nlev+KHALO,1+IHALO:nlon+IHALO,1+JHALO:nlat+JHALO,iv3dd_rho) = v3dg(:,:,:,iv3d_rho)
#endif
...
!-----------------------------------------------------------------------
SUBROUTINE calc_rh_tmp(t,q,p,rh)
  IMPLICIT NONE
...
!-------------------------------------------------------------------------------
#ifdef GPR
subroutine state_to_history_1d(v1dg, v1dgh)
  use scale_grid_index, only: &
      KHALO, KS, KA, KE
  implicit none

  real(RP), intent(in) :: v1dg(nlev,nv1d)
  real(r_size), intent(out) :: v1dgh(nlevh,nv1dd)
  integer :: iv1d

  ! Variables that can be directly copied
  !---------------------------------------------------------

  v1dgh(1+KHALO:nlev+KHALO,iv1dd_lay) = v1dg(:,iv1d_lay)
  v1dgh(1+KHALO:nlev+KHALO,iv1dd_lev) = v1dg(:,iv1d_lev)

  ! Pad the upper and lower halo areas
  !---------------------------------------------------------
  do iv1d = 1, nv1dd
        v1dgh(   1:KS-1,iv1d) = v1dgh(KS,iv1d)
        v1dgh(KE+1:KA,  iv1d) = v1dgh(KE,iv1d)
  end do

  return
end subroutine state_to_history_1d
#endif
