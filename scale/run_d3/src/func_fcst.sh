...
if ((BDY_FORMAT >= 1)); then
  ...
  if [ -z "$PARENT_REF_TIME" ]; then
    ...
    for bdy_startframe in $(seq $BDY_STARTFRAME_MAX); do
      if ((BDY_FORMAT == 1)) && [ -s "$DATA_BDY_SCALE/${PARENT_REF_TIME}/${BDY_SCALE_DIR}/${BDY_MEAN}/history${SCALE_SFX_0}" ]; then
        break
      ...
      if ((MAKEINIT != 1)); then
        for m in $(seq $fmember); do
          ...
          for q in $(seq $mem_np_); do
            ...
            echo "${INDIR}/ref_${PARENT_REF_TIME}/${path}|${OUT_SUBDIR}/${path}" >> ${STAGING_DIR}/${STGINLIST}.${mem2node[$(((mm-1)*mem_np+q))]}
          done
        done
      fi
