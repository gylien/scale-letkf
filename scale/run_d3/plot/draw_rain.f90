...
real,parameter::vsmax_abs=5.5
real,parameter::vsmin_abs=0.5
real,parameter::vsintv_abs=0.5
...
narg = iargc()
if (narg.lt.6) then
 write(*,*) 'input $ctimeparent $ctimebase $cmem $itimestart $itimeend $itimeskip'
 stop
else
...
istatus=nf_inq_varid(idnc,'RAIN',idvarT)
istatus=nf_get_vara_real(idnc,idvarT,(/1,1,itimeref/),(/nlon,nlat,1/),val_mean(:,:))
istatus=nf_close(idnc)

where (abs(val_mean).gt.1e10) val_mean=rmiss

title3='surface rain rate'
title2=''

val_plot_s=val_mean  *3600.0
where (abs(val_mean).gt.1e10) val_plot_s=rmiss
!psfile=trim(cdir_base_fcst)//'ref_'//trim(ctimeparent)//'/'//trim(ctimebase)//'/plot/'//trim(cmem)//'/rain'//cftime
!psfile='figure'
psfile='plot_temp/'//cmem//'/rain'//cftime
iclrmap=12
title1=cmem
vsmax=vsmax_abs
vsmin=vsmin_abs
...
!  end if
   ...
   itpats(1:ntpat+2) = (/ 0, 40,34,28,50,55,62,68,74,80,84,88,92,98/) * 1000+999
   itpats(1) = 0
