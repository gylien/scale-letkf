...
jobid=$(grep 'pjsub Job' fcst_obcx.log.${PARENT_REF_TIME}.${STIME} | grep -v ++ | grep -v + | cut -d ' ' -f6)
#jobid=$(grep 'pjsub Job' fcst_ofp.log | grep -v ++ | grep -v + | cut -d ' ' -f6)
