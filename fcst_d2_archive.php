...
$stimes='';
exec("ls -1 monitor/archive/monitor_plot_d2_*.png" , $stimes ,$ret);
...
$sets[2] = 'JMA_precip/nowcast_d2/archive';
...
###
 ...
 $json_xloc = file_get_contents('json/archive/xlocs_d2_'.$basetime.'.json' );
 $json_yloc = file_get_contents('json/archive/ylocs_d2_'.$basetime.'.json' );
 $json_itime = file_get_contents('json/archive/itimes_d2_'.$basetime.'.json' );
...
if ( intval($date) >= intval(substr($InitTimes[$s][0]),0,4) ){
$path = "$datadir/" . "$sets[$s]". "/$date" ;
$imgs = scandir($path);
foreach ($imgs as $img_file) {
  if (ereg ("radar_([0-9]{10})0000.png", $img_file, $regs) ) {
...
<div align="left" style="position:relative; top:0px; left:0px;">
  <img id="monitor_campus" src="./monitor/archive/monitor_plot_d2_<?php echo $basetime; ?>.png" style="margin:0px;"> 
